...
try:
    import numpy as np
    import pandas as pd
    print("   [OK] NumPy/Pandas imported")
except ImportError as e:
    print(f"   [FAIL] NumPy/Pandas import failed: {e}")
    sys.exit(1)

try:
    from jsf.data import PriceData, SyntheticDataLoader
    print("   [OK] JSF data module imported")
except ImportError as e:
    print(f"   [FAIL] JSF data import failed: {e}")
    sys.exit(1)
...
try:
    ...
    print("   [OK] JSF ML models imported")
except ImportError as e:
    print(f"   [FAIL] JSF ML import failed: {e}")
    sys.exit(1)
...
print(f"   [OK] Created {len(price_data.dates)} days of data for {len(price_data.symbols)} symbols")
...
print(f"   [OK] Extracted {features.shape[1]} features, {features.shape[0]} samples")
...
print(f"   [OK] Aligned data: {len(X)} samples, {X.shape[1]} features")
...
print(f"   [OK] RandomForest trained and predicted {len(rf_pred['returns'])} samples")
...
try:
    ...
    print(f"   [OK] XGBoost trained and predicted {len(xgb_pred['returns'])} samples")
...
try:
    import lightgbm
    print(f"   [OK] LightGBM version: {lightgbm.__version__}")
    ...
    print(f"   [OK] LightGBM trained and predicted {len(lgb_pred['returns'])} samples")
except Exception as e:
    print(f"   [FAIL] LightGBM failed: {e}")
...
try:
    ...
    # Test MLP - use iloc for integer-based slicing on MultiIndex
    mlp = MLPModel(
        hidden_layers=[32, 16],
        dropout_rate=0.2,
        epochs=5,
    ...
    mlp_pred = mlp.predict(X.iloc[train_size:train_size+100])
    print(f"   [OK] MLP trained and predicted {len(mlp_pred['returns'])} samples")

except ImportError as e:
    print(f"   [FAIL] TensorFlow import failed: {e}")
except Exception as e:
    print(f"   [FAIL] TensorFlow neural network failed: {e}")
...
try:
    ...
    print(f"   [OK] FinBERT mock mode works: {mock_result.label.value}")
    ...
    print(f"   [OK] FinBERT REAL mode works: {real_result.label.value} (score: {real_result.score:.2f})")
    ...
    print(f"   [OK] Batch prediction: {len(batch_results)} texts analyzed")
    for i, res in enumerate(batch_results):
        print(f"      [{res.label.value:8}] {texts[i][:50]}...")

except ImportError as e:
    print(f"   [FAIL] BERT import failed: {e}")
except Exception as e:
...
try:
    ...
    print(f"   [OK] TextSentimentSignal generated {result.shape[0]} signals")
    
except Exception as e:
    print(f"   [FAIL] Sentiment signal failed: {e}")
...
All ML models are integrated and working with REAL implementations:
  [OK] XGBoost - Real model (not mock)
  [OK] LightGBM - Real model (not mock)  
  [OK] TensorFlow - Real neural networks
  [OK] FinBERT - REAL transformer model from HuggingFace
  [OK] Sentiment Signals - Integrated with signal framework
